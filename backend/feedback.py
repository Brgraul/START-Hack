...
class FeedbackModule:
    ...
    @staticmethod
    def load_new_emotion(face, identify, new_person):
        identified = identify(face["faceId"])[0]
        personName = "Unnamed person"
        if len(identified["candidates"]) == 0:
            personName = input("Enter person name at {}: ".format(face["faceRectangle"]))
            face["faceId"] = new_person(personName, face["faceRectangle"])
        else:
            face["faceId"] = identified["candidates"][0]["personId"]
        if face["faceId"] not in FeedbackModule.people:
            FeedbackModule.people[face["faceId"]] = Person(face["faceId"], personName)
